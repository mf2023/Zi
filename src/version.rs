...
mod tests {
    ...
    #[test]
    fn create_and_retrieve_versions() {
    ...
    #[test]
    fn compute_digest_differs_for_different_batches() {
    ...
    #[test]
    fn save_with_options_creates_directories_and_roundtrips() {
        let mut store = ZiCVersionStore::ZiFNew();
        let version = store
            .ZiFCreate(
                None,
    ...
    #[test]
    fn load_with_validation_rejects_missing_parent() {
        let record = ZiCVersionRecord {
            id: "v0000000000000001".into(),
            parent: Some("v0000000000000002".into()),
            created_at_secs: 0,
            created_at_nanos: 0,
            metadata: Map::new(),
            metrics: ZiCQualityMetrics::default(),
            digest: "digest".into(),
        };
        let file_payload = ZiCVersionStoreFile {
            next_id: 2,
            versions: vec![record],
        };
        ...
        let mut tmp = NamedTempFile::new().unwrap();
        {
            ...
            serde_json::to_writer(&mut writer, &file_payload).unwrap();
            writer.flush().unwrap();
        }

        let err = ZiCVersionStore::ZiFLoadFromPathWithValidation(tmp.path(), true).unwrap_err();
        match err {
            ZiError::Validation { message } => {
                assert!(message.contains("references missing parent"));
            }
