...
#[test]
fn pipeline_filters_enriches_and_limits_jsonl() {
...
"#;
    ...
    let pipeline = builder
        .build_from_config(config.as_array().expect("config must be array"))
        .unwrap();
    let batch = ZiCIO::ZiFLoadJsonl(input.path()).unwrap();
    let processed = pipeline.run(batch).unwrap();
    ...
    ZiCIO::ZiFWriteJsonl(output.path(), &processed).unwrap();

    let reloaded = ZiCIO::ZiFLoadJsonl(output.path()).unwrap();
    assert_eq!(reloaded.len(), 1);
    assert_eq!(reloaded[0].payload["text"], json!("hello world"));
}
